...
public class B_ifElse {
    Scanner sc = new Scanner(System.in);        //스캐너 클래스를 사용하기 위한 조건
    ...
    public void testSimpleIfElseStatement() {       //void는 바로 출력해주는데 리턴값이 없다.
        System.out.print("정수 한 개를 입력하시오.");
        int num = sc.nextInt();

        /*필기. 짝수인지 조건 확인 */
        if (num % 2 != 0) {
            System.out.println("입력한 정수는 홀수다.");
        } else {
            System.out.println("입력한 정수는 짝수다.");
        }
    ...
    public void testNestedIfElseStatement() {
        ...
        /* 필기.
         *   숫자를 하나 입력 받아 양수이면 "입력한 숫자는 양수이다."
         *   음수이면 "입력한 숫자는 음수이다" 출력
         *   단, 0이면 "0입니다"   라고 출력하세요
         *   */
        ...
        if (num != 0) {
            if (num > 0) {
                System.out.println("입력한 숫자는 양수다.");
            } else {
                System.out.println("입력한 숫자는 음수다.");
            }
        } else {
